#ifndef BBP_TEXT_FILE_READER_HPP_
#define BBP_TEXT_FILE_READER_HPP_
#include "SpatialObjectStream.hpp"
#include "BufferedFile.hpp"
#include "Box.hpp"

using namespace std;

namespace FLAT
{
	/*
	 * Class Responsible for Handling the Input Data
	 * It Reads the Header info
	 * It Reads items from File into memory and returns
	 */
	class BBPTextFileReader :public SpatialObjectStream
	{
	public:
		SpatialObject* data;
		std::ifstream inputFile;
		//BufferedFile* inputFile;      //Pointer to file containing input Data
		uint64 counter;

		BBPTextFileReader(string& segmentFileName,string& sliceFileName);

		~BBPTextFileReader();

		void loadHeader(string& filename);

		bool hasNext();

		SpatialObject* getNext();

		void rewind();
	};
}

#endif
