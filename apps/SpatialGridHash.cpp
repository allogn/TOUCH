...
SpatialGridHash::SpatialGridHash(const Box& universeExtent,const int gridResolutionPerDimension)	
{
        initialize.start();
        ...
        gridSize = (uint64)(resolution * resolution) * resolution;
...
//		cout << "size(" << gridHashTable.size() << ") of the largest possible container: " << gridHashTable.max_size();
        ...
        footprint += sum*sizeof(SpatialObject*) +  sizeof(HashValue)*gridSize;
