...
dTOUCH::dTOUCH(unsigned int buckets)
{
        initialize.start();
...
void dTOUCH::assignmentA(const SpatialObjectList& ds)
{
        building.start();
        int overlaps;
        int assigned_level;
        ...
        for (unsigned int i=0;i<ds.size();++i)
        {
                ...
                while(true)
                {
                        ...
                        if(assigned)
                        {
                                obj->cost = overlaps*((pow(partition,assigned_level+1)-1)/(partition - 1) - 1);
                                break;
                        }
                        if(!overlaps)
                        {
...
void dTOUCH::joinIntenalnodetoleafs(uint64 ancestorNodeID, vector<TreeNode*>& tree, TreeEntry* root)
{
        ...
        while(leaves.size()>0)
        {
                ...
                if(leaf->leafnode)
                {
                        ...
                        if(localJoin == algo_SGrid)// && localPartitions < internalObjsCount)// && internal->level >0)
                        {
                                ...
                                {
                                        resultPairs.addPair(it->first,it->second);
                                }
                        }
                        else
...
void dTOUCH::analyze(const SpatialObjectList& dsA,const SpatialObjectList& dsB)
{
        ...
        footprint += sum*sizeof(SpatialObject*) + treeB.size()*(sizeof(TreeNode*)) + treeA.size()*(sizeof(TreeNode*));
