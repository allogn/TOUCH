...
public:
							...
							for (unsigned int cChild = 0; cChild < ptr->entries.size(); ++cChild)
							{
								...
								if ( Box::overlap(objMBR,ptr->entries.at(cChild)->mbrK[current_type]) ) // here mbr is dark+light
								{
									...
									else
									{
										...
										obj->cost += pow(NODE_FANOUT,ptr->level);
	...
	void joinInternalobjecttodesc(SpatialObject* obj, uint64 ancestorNodeID, ResultPairs& results)
	{
		queue<uint64> nodes;
		int nodeID;
		int opType;
		...
		while(nodes.size()>0)
		{
			...
			for (uint64 child = 0; child < node->entries.size(); ++child)
			{
				...
				if (Box::overlap(obj->getMBR(), node->entries.at(child)->mbrD[opType]))
				{
					...
					JOIN(obj, downnode->attachedObjs[opType], results);
	...
	void joinIntenalnodetoleafs(uint64 ancestorNodeID, ResultPairs& results)
	{
		...
		{
			joinInternalobjecttodesc((*it), ancestorNodeID, results);
		}
		...
		if (node->attachedObjs[0].size() < node->attachedObjs[1].size())
		{
			for (SpatialObjectList::iterator it = node->attachedObjs[0].begin();
									it != node->attachedObjs[0].end(); it++)
			{
				JOIN((*it), node->attachedObjs[1], results);
			}
		}
		else
		{
			for (SpatialObjectList::iterator it = node->attachedObjs[1].begin();
						it != node->attachedObjs[1].end(); it++)
			{
				JOIN((*it), node->attachedObjs[0], results);
			}
		}
	...
	void Oprobe(ResultPairs& results)
	{
		...
		while(Qnodes.size()>0)
		{
			...
			joinIntenalnodetoleafs(currentNodeID, results); //join node -> join each object -> join object to down tree -> join object with list of objects
	...
	void analyze(const SpatialObjectList& dsA,const SpatialObjectList& dsB)
	{
		...
		statsTOUCH.footprint += vdsAll.size()*(sizeof(TreeEntry*));
