...
allobjnum = np.unique(np.array(alldata[:,7],dtype=float))

additional = "E = " + str(epsilon) + "; Local Join: " + alldata[0,6] + "; ObjNum: " + \
              alldata[0,3] + "; Leaf size: " + alldata[0,8] + "; Grid Size: " + alldata[0,9] + \
...
for col in allcols:
    ...
    for alg in allalg:
        ...
        for objnum in allobjnum:
            objdata = algdata[np.array(algdata[:,7],dtype=float)==objnum]
